		...
		<div class="user">
			<div class="top">
				...
				<div class="switch">
					...
					<div class="switch-item" :class="{active:selected==='playHistory'}" @click="switchItem('playHistory')">播放历史</div>
				</div>
			</div>
			<div class="container-item" v-show="selected==='mylikeSongs'">
				...
				<div class="play-btn" v-show="mylikeSongs.length" @click="randomPlay(mylikeSongs,'mylikeSongs')">
					<i class="iconfont icon-play_icon mr-2"></i>
					<span>播放全部</span>
				</div>
				<ul class="list">
					<li v-for="(item,index) in mylikeSongs" class="item" :key="index" @click="selectItem(mylikeSongs,index,'mylikeSongs')">
						<h3 :class="active(item,'mylikeSongs')">{{item.songname}}</h3>
						<p :class="active(item,'mylikeSongs')">{{item.singername}}--{{item.albumname}}</p>
			...
			<div class="container-item" v-show="selected==='playHistory'">
				...
				<ul class="list">
					<li v-for="(item,index) in playHistory" class="item" :key="index" @click="selectItem(playHistory,index,'playHistory')">
						<h3 :class="active(item,'playHistory')">{{item.songname}}</h3>
						<p :class="active(item,'playHistory')">{{item.singername}}--{{item.albumname}}</p>
